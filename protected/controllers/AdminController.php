   ...
   public function actionGetfoldercontent() {
    	if(isset($_GET['id']) && isset($_GET['type']))
    	{
	    	...
	    	if($type == 'treeGeneral')
	    	{
		    	...
		    	$sql = "SELECT `requester`, `target` FROM `relations` WHERE `type` = 'fileToFolder' " .
    ...
    public function actionView() {
    	if(isset($_POST['id']) && isset($_POST['type']))
    	{
    		...
    		if($type == 'treeGeneral')
    		{
	    		...
	    		$sql = "SELECT `requester`, `target` FROM `relations` WHERE `type` = 'fileToFolder' AND " . 
	    				"`target` = '".$folderid."' AND ".
	    				"`allowedBy` = '".$adminName."';";
	    		$command = $connection->createCommand($sql);
	    		$dataReader=$command->query();
