...
<!-- 	</div> -->
	...
	<!-- <div class="row">
		<?php //echo $form->labelEx($model,'TV'); ?>
		<div id="radioSet">
		    <input type="radio" id="radio1" class='radioTv' name="tv" value='1'>
		    	<label for="radio1">On</label>
		    <input type="radio" id="radio2" class='radioTv' name="tv" value='0' checked="checked">
 	...
 	<div class="row" style='display:none;'>
		<input id="tvRadioVal" name="tv" type='text'>
	</div> 
-->
	<div class="row">
	<?php 
				...
				if($ScreenModelId != null){
					...
					if(isset($ScreenModel->windows)) {
						...
						foreach ($windows as $window) {
							$windowId = $window->id;
							$WindowModel = Window::model()->findByPk($windowId);
							$windowName = $WindowModel->name;
							$channels = $WindowModel->channels;
							
							foreach ($channels as $channel) {
								if($channel->id_point == $model->id) {
									...
									printf ("<button type='button' class='AddPlaylistsBut btn btn-info' data-channelid='%s'>" . 
										"<span class='glyphicon glyphicon-plus'></span> Add playlists" . "</button></div>", 
										$channel ['id']);
